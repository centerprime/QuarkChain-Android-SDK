package com.centerprime.quarkchainsdk.quarck.rlp;
...
public class RlpString implements RlpType {
    ...
    public static RlpString createShard(BigInteger value) {
        // RLP encoding only supports positive integer values
        byte[] bytes = value.toByteArray();
        if (bytes.length < 4) {
            ...
            return new RlpString(newBytes);
        } else if (bytes[0] == 0) {  // remove leading zero
            return new RlpString(Arrays.copyOfRange(bytes, 1, bytes.length));
        } else {
            return new RlpString(bytes);
        }
